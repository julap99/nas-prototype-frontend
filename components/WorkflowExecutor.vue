...
const initializeVariables = () => {
  try {
    console.log('[Workflow] Initializing variables from props...');
    
    // Set initial variables from props
    if (props.initialVariables && Object.keys(props.initialVariables).length > 0) {
      Object.assign(processVariables.value, props.initialVariables);
      console.log('[Workflow] Initialized variables:', processVariables.value);
    }
  } catch (err) {
    console.error('[Workflow] Error initializing variables:', err);
  }
};
...
<template>
  <div :class="isIframeMode ? 'min-h-screen' : 'min-h-screen bg-gray-50'">
    <!-- Header - Hidden in iframe mode or when showHeader is false -->
    <header v-if="!isIframeMode && props.showHeader" class="bg-white border-b border-gray-200 px-6 py-4 shadow-sm">
